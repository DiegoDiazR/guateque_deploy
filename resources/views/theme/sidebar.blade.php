<div class="sidebar-nav slimscrollsidebar">
    <div class="sidebar-head">
        <h3><span class="fa-fw open-close"><i class="ti-menu hidden-xs"></i><i class="ti-close visible-xs"></i></span> <span class="hide-menu">Principal</span></h3>
    </div>
    <ul class="nav" id="side-menu">
        <li> <a href="/profile" class="waves-effect"><i  class="fa fa-home"></i> <span class="hide-menu">Inicio</span></a> </li>
        @foreach (Session::get('opciones') as $opcion)
        <li id="li_parent_{{ $opcion->id }}" {{ $opcion->opcion_padre != null ? 'class=opcion' : 'class=padre' }} parent="{{ $opcion->opcion_padre != null ? $opcion->opcion_padre : $opcion->id }}">
            @if($opcion->opcion_padre != null)
            <a href="/{{ $opcion->url }}/{{ base64_encode($opcion->id) }}" class="waves-effect">
                <i class="{{ $opcion->icono }}"></i> <span class="hide-menu">{!! $opcion->titulo !!}</span>
            </a>
            @else
            <a href="javascript:void(0)" class="waves-effect">
                <i class="{{ $opcion->icono }}"></i> <span class="hide-menu">{!! $opcion->titulo !!}<span class="fa arrow"></span></span>
            </a>
            @endif
        </li>
        @endforeach
